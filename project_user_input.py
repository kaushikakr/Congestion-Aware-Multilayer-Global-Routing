...
def askTheUser():
    ...
    ll_user_x = float(0.3) #input("Enter lower bound for X includes float numbers as well: ")
    ur_user_x = float(923.5)#input("Enter upper bound for X includes float numbers as well: ")
    ll_user_y = float(0.084)#input("Enter lower bound for Y includes float numbers as well: ")
    ur_user_y = float(978.4) #input("Enter upper bound for Y includes float numbers as well: ")
...
with open('input_file_10000_nets.json', 'w') as f:
    json.dump(data, f)
