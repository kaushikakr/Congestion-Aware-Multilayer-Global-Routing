...
def visualize(paths):
    ...
    for i in range(len(paths)):
        for j in paths[i]:
            ...
            if(j in num_used_tracks.keys()):
                grid.at[j, "edge_congestion"] = (
                    1/ov)*(num_used_tracks[j]/capacity["M"+str(j[0])])
            else:
    ...
    f1, i1 = create_Fig(np.reshape(image1, (-1, num_cols)))
    ...
    f3, i3 = create_Fig(np.reshape(image3, (-1, num_cols)))
    ...
    f4, i4 = create_Fig(np.reshape(image4, (-1, num_cols)))
    ...
    f5, i5 = create_Fig(np.reshape(image5, (-1, num_cols)))
    ...
    f6, i6 = create_Fig(np.reshape(image6, (-1, num_cols)))
...
input_json_file = "input_file_1000_nets.json"
global weight
...
for j in range(len(paths)):
    ...
    fileOperations.write_to_jsonfile(
        folder + "/GR_bucket_" + folder + "_" + timestr + ".json", GR_bucket_data)
